...
body<-bs4DashBody(
  bs4TabItems(
    bs4TabItem(tabName = "mining",
               ...
               fluidRow(
                 bs4Card(title = "Input", status = "primary", solidHeader = TRUE, width=4,
               ...
               fluidRow(
                 bs4Card(title = "Download Full Data", status = "primary", solidHeader = TRUE, width = 4,
                     # tags$h4("Click the Below Button to download Data"),
                     downloadButton(ns('downloadData'), 'Download'))),
               
               fluidRow( 
                 bs4Card(title = "Tabulated Results", status = "info", solidHeader = TRUE, width = 12,
    ...
    bs4TabItem(tabName = "arab",
               ...
               fluidRow(
                 ...
                 bs4Card(title = "Top Hashtags", status = "primary", solidHeader = TRUE, width=6,
                     plotOutput(ns("topHashtags"))),
                 bs4Card(title = "Top Active Users", status = "warning", solidHeader = TRUE, width=6,
                     plotOutput(ns("topUsers"))),
                 bs4Card(title = "Top Replied To", status = "success", solidHeader = TRUE, width=6,
                     plotOutput(ns("topRepliedTo"))),
                 bs4Card(title = "Twitting Time", status = "danger", solidHeader = TRUE, width=6,
                     plotOutput(ns("plotTime"))),
                 bs4Card(title = "Used Platform", status = "info", solidHeader = TRUE, width=6,
                     plotOutput(ns("device"))),
                 bs4Card(title = "Sentiment Analysis", status = "primary", solidHeader = TRUE, width=6,
                     plotlyOutput(ns("sentPie"))),
                 bs4Card(title = "Sentiment by Time", status = "warning", solidHeader = TRUE, width=6,
                     plotOutput(ns("arabSentiment")))
                 # bs4Card(title = "Cloud of Top 100 Words", status = "success", solidHeader = TRUE, width=6,
                 #     plotOutput("cloud100"))
    ...
    bs4TabItem(tabName = "tables",
               ...
               fluidRow(
                 column( width=6,
                 bs4Card(title = "Top Negative Words", status = "info", solidHeader = TRUE, width=12,
                     DT::DTOutput(ns("topNeg"))),
                 bs4Card(title = "Top Favorites & Retweets", status = "danger", solidHeader = TRUE, width=12,
                         DT::DTOutput(ns("topFavRetw"))),
                 bs4Card(title = "Levels of Sentiment", status = "success", solidHeader = TRUE, width=12,
                         DT::DTOutput(ns("threeEmo"))),
                 
                 ),
                 column(width=6,
                   bs4Card(title = "Top Positive Words", status = "primary", solidHeader = TRUE, width=12,
                           DT::DTOutput(ns("topPos"))),
                 bs4Card(title = "Most Frequent Words", status = "warning", solidHeader = TRUE, width=12,
                     DT::DTOutput(ns("topWords"))),
                 bs4Card(title = "Full Data", status = "info", solidHeader = TRUE, width=12,
                     DT::DTOutput(ns("allEmo")))
                 )
               )
    ),
    bs4TabItem(tabName = "sna", 
               ...
               div(
                 tabBox(    
                   ...
                   id = "summaries", width = 12,
                   tabPanel("Retweet Network",
                            fluidRow(
                              bs4Card(title = "Retweet Network", status = "warning", solidHeader = TRUE, width=10, 
                                  
                                  forceNetworkOutput(ns("sna")))),
                            fluidRow(
                              bs4Card(title = "Download Retweet Network Data", status = "info", solidHeader = TRUE, width=10,
                                  tags$h5("Click the Below Button to download .graphml"),
                                  downloadButton(ns('downloadGraphml2'), 'Download'))),
                            fluidRow(
                              bs4Card(title = "Download Node Attributes", status = "info", solidHeader = TRUE, width=10,
                   ...
                   tabPanel("Mention Network", 
                            fluidRow(
                              bs4Card(title = "Mention Network", status = "warning", solidHeader = TRUE, width=10, 
                                  forceNetworkOutput(ns("sna2")))),
                            fluidRow(
                              bs4Card(title = "Download Mention Network Data", status = "info", solidHeader = TRUE, width=10,
                                  tags$h5("Click the Below Button to download .graphml"),
