...
dashTweet_UI <- function(id,side,head,cont,body) {
  ns <- NS(id)
  bs4DashPage(header = head,sidebar = side,body = body,controlbar = NULL,title = "TwitterLycs",dark = TRUE,fullscreen = T)
}

dashTweet <- function(input, output, session,user_token,keys,pos.words,neg.words, stop.wordsAr) {
  observe(print(keys))
  ...
  # # access_token <- "271065604-cY4IuIeqBC3Isl3YNdXMpZHrmHE6SlHriaNCLTLl"
  # # access_secret <- "uzkDLrrtNkQqMJVYr43LtBrmoquAgC1tO69HITxP0jQag"
  ...
  data <- shiny::isolate({eventReactive(input$update, {
    #$$$$
    withProgress({
  ...
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
  ## Top retweeted tweet
  #  output$topRetweeted <- DT::renderDT({
  #    
